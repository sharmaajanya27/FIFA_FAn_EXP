...
export function VenueDetail({ venue, city, onClose }: { venue: RankedVenue; city: string; onClose: () => void }) {
  ...
  const reportCrowd = (level: CrowdLevel) => guard(async () => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 2000 }} onClick={onClose}>
      <aside
        className="panel"
        style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "min(460px, 100%)", borderRadius: 0, overflowY: "auto" }}
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="row" style={{ position: "sticky", top: 0, background: "var(--panel)" }}>
        ...
        <div className="rec">
          ...
          <div className="badges" style={{ marginTop: 8 }}>
            ...
            <span className="badge">match score {Math.round(venue.finalScore * 100)}</span>
          </div>
        </div>
        ...
        <div className="rec">
          <button className="primary" disabled={busy} onClick={checkIn}>📍 Check in here</button>
        </div>
        {checkins.slice(0, 5).map((c) => (
          <div className="event" key={c.id}>
            <strong>{c.userName}</strong> <span className="muted">checked in</span>
        ...
        <div className="rec">
          <div className="row" style={{ justifyContent: "flex-start", gap: 8 }}>
            ...
            <input style={{ flex: 1 }} value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Share your experience…" />
            <button className="primary" disabled={busy} onClick={submitReview}>Post</button>
          </div>
        </div>
        {reviews.slice(0, 8).map((r) => (
          <div className="event" key={r.id}>
            <div><strong>{r.userName}</strong> <span className="badge">{"★".repeat(r.rating)}</span></div>
        ...
        <div className="rec" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {photos.map((p) => (
            // eslint-disable-next-line @next/next/no-img-element
            <img key={p.id} src={p.dataUrl} alt={p.caption ?? "fan photo"} width={84} height={84} style={{ objectFit: "cover", borderRadius: 8, border: "1px solid var(--border)" }} />
          ))}
