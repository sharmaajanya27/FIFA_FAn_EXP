# FanFndr — Product Requirements Document
...
FanFndr is a location-based web application that helps soccer fans discover the
...
FanFndr should **not** be positioned only as a "World Cup" app. It launches
...
sites, and local communities. **FanFndr centralizes it into one platform** — a
...
FanFndr runs an automated ingestion layer that **scrapes and aggregates** the
...
2. **Normalize** — map disparate schemas into the canonical FanFndr model.
